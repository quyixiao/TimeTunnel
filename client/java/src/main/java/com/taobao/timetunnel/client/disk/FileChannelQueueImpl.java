...
public class FileChannelQueueImpl implements FileChannelQueue {
	...
	private final ByteBuffer writeLenBuffer = ByteBuffer.allocate(4);
	...
	private final PreAllocate preAllocate;
	...
	public FileChannelQueueImpl(final String queueName) {
		...
		this.preAllocate = new PreAllocate();
	...
	private void checkIfHasIncompleteContent() {
		if (lenBuffer.position() != 0 || cb.isActive()) {
			String errMsg = "wrong file content occurred in " + readFile + " at " + readPos+" maybe due to abnormal close last time";
			log.error(errMsg);
			lenBuffer.clear();
			cb.clear();
		}
	...
	private int getAMessage(int lastLen) {
		int len = lastLen;
		...
		log.debug("content buffer " + cb.position() + " and " + rawData.remaining() + " and len is: " + cb.getLimit());
		int cr = cb.remaining();
		for (int i = 0; i < cr; i++)
			cb.put(rawData.get());
		cb.flip();
...
//		log.info("add to q elipse: " + (System.nanoTime() - begin));
	...
	private void write(Message m) {
		try {
			...
			content.put(writeLenBuffer);
			writeLenBuffer.clear();
			content.put(c, 0, c.length);
			content.flip();
			writeFileChannel.write(content);
		} catch (Exception e) {
